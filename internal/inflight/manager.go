...
//   - phase: 연결 상태 ("P"=Primary, "S"=Secondary)
//
// 반환값: 생성된 InflightEntryA (ResponseChan에서 응답 대기용)
//
// 주의: 반환된 entry의 ResponseChan은 반드시 select로 수신해야 합니다.
//       timeout 처리를 위해 TimeoutChan도 함께 모니터링하세요.
func (ia *InflightA) Register(tid uint32, replySubject string, payload []byte, deadline int64, phase string) *InflightEntryA {
...
func (ia *InflightA) cleanupExpired() {
	...
	if len(expiredTIDs) > 0 {
		...
		for _, tid := range expiredTIDs {
			if entry, exists := ia.entries[tid]; exists {
				// Notify timeout
				select {
