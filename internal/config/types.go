...
// InflightEntry represents an inflight request entry
type InflightEntry struct {
	TID         uint32    // Transaction ID
	ReplySubject string    // NATS reply subject (for Inflight-A)
	TCPReplyInfo *TCPReplyInfo // TCP reply info (for Inflight-B)
	Payload     []byte    // Request payload
	Attempt     int       // Current attempt number
	Deadline    int64     // Request deadline (unix timestamp)
	Phase       string    // Connection phase (P=Primary, S=Secondary)
}
...
func (f *Frame) IsRequest() bool {
	// Request는 홀수 타입 (0x01, 0x03, 0x05, 0x07, 0x09, 0x0b)
	return f.Type%2 == 1
}

// IsResponse returns true if the message is a response type
func (f *Frame) IsResponse() bool {
	// Response는 짝수 타입 (0x02, 0x04, 0x06, 0x08, 0x0a, 0x0c)
	return f.Type%2 == 0 && f.Type != 0
}

// IsNATSToTCPRequest checks if the frame is NATS→TCP request
// (0x01, 0x03, 0x05, 0x09: Hello, Ping, Subs-Change, CellInfo-Noti)
func (f *Frame) IsNATSToTCPRequest() bool {
	return f.Type == MsgTypeHelloRequest ||
		f.Type == MsgTypePingRequest ||
		f.Type == MsgTypeSubsChangeRequest ||
		f.Type == MsgTypeCellInfoNotiRequest
}

// IsTCPToNATSRequest checks if the frame is TCP→NATS request
// (0x07, 0x0b: Subs-Info, Subs-Sync)
func (f *Frame) IsTCPToNATSRequest() bool {
	return f.Type == MsgTypeSubsInfoRequest ||
		f.Type == MsgTypeSubsSyncRequest
}

// GetResponseType returns the corresponding response type for a request
