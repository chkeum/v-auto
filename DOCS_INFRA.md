...
## 1. 인프라 디렉토리 구조 (`infrastructure/`)

v2.0부터 모든 인프라 관련 설정은 `infrastructure/` 폴더에 격리되었습니다. 일반 사용자는 이 폴더를 수정할 권한이 없습니다.

```text
infrastructure/
├── networks.yaml    # 네트워크 토폴로지 정의
├── images.yaml      # 골든 이미지 카탈로그
├── storage.yaml     # 스토리지 클래스 정의
└── templates/       # Kubernetes 리소스(Jinja2) 템플릿
```
