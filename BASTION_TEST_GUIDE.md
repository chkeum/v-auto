...
### 1-2-1. Git 작업 전체 자동화 (sync.sh)
매번 `add`, `commit`, `push`를 입력하기 번거롭다면, 다음 스크립트로 한 번에 처리할 수 있습니다.

```bash
# 스크립트 실행 권한 부여
chmod +x sync.sh

# 모든 변경 사항(추가/수정/삭제)을 커밋하고 푸시
./sync.sh "어떤 부분을 수정했는지 적어주세요"
```

> [!NOTE]
> **스크립트 역할 구분**
> - **`sync.sh`**: 소스 코드 수정, 가이드 업데이트 등 **일반적인 작업 내용**을 Git에 저장할 때 사용합니다.
> - **`bundle.sh`**: 현장 반입을 위한 **최종 압축 파일(tar.gz)을 생성**하고 배포할 때 사용합니다.

> [!TIP]
> **매번 `git add` 하기가 귀찮다면? (단축 명령어)**
> 이미 Git이 관리하고 있는 파일(예: `vm_manager.py`)만 수정했다면, add 단계를 건너뛰고 바로 커밋할 수 있습니다:
> `git commit -am "수정 메시지"`
> - **`-a` 옵션**: 변경된(Modified) 파일을 자동으로 add 해줍니다.
> - **주의**: `bundle.sh`처럼 **처음 만든 새 파일(Untracked)**은 이 명령어로 추가되지 않으니, 새 파일이 있을 때는 꼭 `git add .`를 먼저 해주세요!
